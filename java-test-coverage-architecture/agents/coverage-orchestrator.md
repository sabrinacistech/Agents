...
5. Envolver cada ciclo en `cycle_loop.py` — **el único dueño del loop**. Es la
   ...
   ciclo, así "no medido" nunca se confunde con un delta previo. **El comando
   envuelto debería producir `state/coverage-delta.json`** (vía
   `narrow_test_runner` + JaCoCo) para que un ciclo plano real se cuente; si no
   lo produce, el ciclo se PRESERVA (no dispara un falso stall, a diferencia del
   comportamiento previo). El `--auto-repair` de `gate_runner` vive *dentro* de
   ese comando de ciclo.
