...
(`<proyecto>/src/test/java/…`); la arquitectura **nunca** toca `src/main`. Esta separación garantiza
que el repo objetivo solo reciba tests, y que todo el ruido de ejecución quede aislado y borrable.
El path se controla con `--out` (pre-stage) / `--state` (patcher y runner), o `-StateDir`
(`run_agents.ps1`). Ver el paso a paso en [§Ejecución desde VS Code + Copilot](#ejecución-desde-vs-code--copilot-paso-a-paso).
...
Escenario: en VS Code tenés abierta **esta arquitectura** como workspace, y disparás la cobertura
de un proyecto externo con **un prompt en Copilot Chat**. Todo lo que genera la arquitectura queda
**fuera** del proyecto; solo los tests se escriben **dentro** del proyecto.
...
C:\repo\Agents\java-test-coverage-architecture   ← workspace abierto en VS Code (esta arquitectura).
```
...
Pegá esto en Copilot Chat con la arquitectura como workspace, reemplazando las 3 variables del
encabezado:

```text
Actuá como Orchestrator de la arquitectura java-test-coverage-architecture (este workspace).
...
> Para el flujo **VS Code + Copilot con estado externo** (workspace = esta arquitectura, proyecto y
> carpeta `coverage_<proyecto>` por parámetro), seguí directamente
> [§Ejecución desde VS Code + Copilot](#ejecución-desde-vs-code--copilot-paso-a-paso). Los pasos de
> abajo son la vista general genérica.
...
3. Correr el pre-stage Python apuntando `--out` a la carpeta externa `coverage_<proyecto>` (auto-detección: `python tools/python/bootstrap.py --repo <proyecto> --out C:/repo/coverage_<proyecto>`; overrides manuales con `run_pipeline.py`).
