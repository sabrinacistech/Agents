...
finiteness holds regardless of who drives generation. Each cycle:
  ...
  4. record outcome — write the TWO fields G8 reads, derived deterministically:
                        - consecutiveZeroDeltaCycles: +1 only when the cycle
                          MEASURED coverage and it was flat; reset to 0 on
                          measured progress; PRESERVED (unchanged) when the cycle
                          produced no fresh coverage-delta.json — a structural
                          skip/block, a missing-baseline no-op or a compile
                          failure. Counting those no-ops was the M3 false stall
                          that forced full pipeline restarts. A stale-guard
                          deletes coverage-delta.json before each cycle so "no
                          measurement" is never confused with a previous delta.
