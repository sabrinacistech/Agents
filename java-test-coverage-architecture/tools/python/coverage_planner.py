...
----------------------------------------------
  ...
  Targets whose SUT is classified ``generated/excluded`` (OpenAPI/CXF/codegen,
  per classification_analyzer.py Rule 1) are dropped BEFORE scoring — generated
  artifacts are never valid coverage targets regardless of their miss counts.
